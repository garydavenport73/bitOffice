...
clearCalendarFormEntries();calendarTable.innerHTML=buildCalendarTableElement(date);showPlannerDiv("planner-calendar-table");}
...
function addSortValueToCalendarData(){for(let i=0;i<calendarDatabase["data"].length;i++){let row=calendarDatabase["data"][i];if((row["Start Date"]==="")||(row["Start Date"]===undefined)){row["Start Date"]="1970-01-01";}
if((row["Start Time"]==="")||(row["Start Time"]===undefined)){row["Start Time"]="00:00";}
...
clearContactFormEntries(contactsTable);contactsTableElement.innerHTML=buildContactsTableElement(contactsTable);showPlannerDiv("planner-contacts-table");}
...
clearContactFormEntries(contactsTable);contactsTableElement.innerHTML=buildContactsTableElement(contactsTable);makeCalendar();};fileReader.readAsText(inputFile,"UTF-8");});inputTypeIsFile.click();}
function saveCombinedDatabase(){purgeCalendar();let str=JSON.stringify(combinedDatabase);let baseFilename="bitOfficePlanner"+getTodaysDate();copyAndSaveString(str,baseFilename,".bof");}
