...
let vEvent=makeVEvent(row,"\n");let eventsArr=[];eventsArr.push(vEvent);let vCal=makeVCalendar(eventsArr,"\n");let filename="vEvent"+row["Start Date"]+"_"+row["Start Time"].replace(/\:/g,"");copyAndSaveString(vCal,filename,".ical",mimeType="text/calendar");if(index>=0){calendarDatabase["data"][index]=row;}else{calendarDatabase["data"].push(row);}
...
let UID="bitOffice."+ISOStringToICalStringZulu((htmlDateAndTimeToISOString(startDate,startTime)))+Math.random().toString();while(usedIDs.includes(UID)){UID="bitOffice."+ISOStringToICalStringZulu((htmlDateAndTimeToISOString(startDate,startTime)))+Math.random().toString();}
return UID;}
...
if(getTodaysDate()===thisDate){entry.style["border"]="2px solid black";entry.style["font-weight"]="bold";}}}
function createOutlookCSV(){let outlookCSV=JSONToCSV(calendarDatabase,true,"\r\n");console.log(outlookCSV);copyAndSaveString(outlookCSV,"plannerToOutlookExport"+getTodaysDate(),".csv","text/csv");}
...
let vCard=makeSingleVCFString(row);copyAndSaveString(vCard,row["First Name"]+"-"+row["Last Name"]+"-"+"Contact"+"-"+getTodaysDate(),".vcf","text/vcard");if(index>=0){contactsTable["data"][index]=row;}else{contactsTable["data"].push(row);}
...
function saveOutlookContactsCSV(){let thisCSV=makeCSV(contactsTable,true);copyAndSaveString(thisCSV,contactsTable["name"]+getTodaysDate(),".csv","text/csv");}
...
else if(line.slice(0,7)==="DTSTART"){let iCalStartDate=line.split(":")[1];let ISOString=iCalDateToISOString(iCalStartDate);let startDateObject=new Date(ISOString);let year=startDateObject.getFullYear().toString();let month=("0"+((startDateObject.getMonth()+1).toString())).slice(-2);let date=("0"+(startDateObject.getDate().toString())).slice(-2);let hours=("0"+(startDateObject.getHours().toString())).slice(-2);let minutes=("0"+(startDateObject.getMinutes().toString())).slice(-2);let seconds=("0"+(startDateObject.getSeconds().toString())).slice(-2);console.log("year: ",year," month: ",month," date: ",date);console.log("hours: ",hours," minutes: ",minutes," seconds: ",seconds);row["Start Date"]=year+"-"+month+"-"+date;row["Start Time"]=hours+":"+minutes;}
else if(line.slice(0,5)==="DTEND"){let iCalEndDate=line.split(":")[1];let ISOString=iCalDateToISOString(iCalEndDate);let endDateObject=new Date(ISOString);let year=endDateObject.getFullYear().toString();let month=("0"+((endDateObject.getMonth()+1).toString())).slice(-2);let date=("0"+(endDateObject.getDate().toString())).slice(-2);let hours=("0"+(endDateObject.getHours().toString())).slice(-2);let minutes=("0"+(endDateObject.getMinutes().toString())).slice(-2);let seconds=("0"+(endDateObject.getSeconds().toString())).slice(-2);row["End Date"]=year+"-"+month+"-"+date;row["End Time"]=hours+":"+minutes;}
else{extraStuff+=line+"\n";}}
row["Description"]+=extraStuff;return row;}
function iCalDateToISOString(icalDate){let year=icalDate.slice(0,4);let month=icalDate.slice(4,6);let day=icalDate.slice(6,8);let hour="00";let min="00";let sec="00";if(icalDate.indexOf("T")!==-1){hour=icalDate.slice(9,11);min=icalDate.slice(11,13);sec=icalDate.slice(13,15);}
if(icalDate.indexOf("Z")!==-1){let ISOString=year+"-"+month+"-"+day+"T"+hour+":"+min+":"+sec+".000Z";return ISOString;}
else{let intYear=parseInt(year);let intMonth=parseInt(month);let intDay=parseInt(day);let intHour=parseInt(hour);let intMin=parseInt(min);let intSec=parseInt(sec);let dateObject=new Date(intYear,intMonth-1,intDay,intHour,intMin,intSec);return dateObject.toISOString();}}
...
console.log(str);copyAndSaveString(str,"vcf"+contactsTable["name"]+getTodaysDate(),".vcf","text/vcard");}
...
clearContactFormEntries(contactsTable);contactsTableElement.innerHTML=buildContactsTableElement(contactsTable);makeCalendar();showPlannerDiv('planner-calendar-start');};fileReader.readAsText(inputFile,"UTF-8");});inputTypeIsFile.click();}
function saveCombinedDatabase(){purgeCalendar();let str=JSON.stringify(combinedDatabase);let baseFilename="bitOfficePlanner"+getTodaysDate();saveStringToTextFile(str,baseFilename,".json","application/json");}
...
let description=row["Description"];let nowDate=new Date();let nowString=nowDate.toISOString();nowString=ISOStringToICalStringZulu(nowString);str+="BEGIN:VEVENT"+lt;str+="SUMMARY:"+subject.trim()+lt;rowDateTimeToICALString(startDate,startTime);rowDateTimeToICALString(endDate,endTime);str+="DTSTART:"+rowDateTimeToICALString(startDate,startTime).trim()+lt;str+="DTEND:"+rowDateTimeToICALString(endDate,endTime).trim()+lt;str+="DESCRIPTION:"+description.trim()+lt;str+="DTSTAMP:"+nowString.trim()+lt;str+="UID:"+UID.trim()+lt;str+="END:VEVENT";return str;}
function rowDateTimeToICALString(date,time){let thisDateISO=htmlDateAndTimeToISOString(date,time);let icalString=ISOStringToICalStringZulu(thisDateISO);return icalString;console.log("fromhtml:");icalStr=date.split("-").join("")+"T"+time.split(":").join("");console.log(icalStr);return icalStr;}
function htmlDateAndTimeToISOString(date="2023-01-31",time){if((time==="")||(time===undefined)){time="00:00";}
let year=parseInt(date.slice(0,4));let month=parseInt(date.slice(5,7))-1;let day=parseInt(date.slice(8));let hour=parseInt(time.slice(0,2));let min=parseInt(time.slice(3));let d=new Date(year,month,day,hour,min);console.log("html date and time to iso string here !!!!",d.toISOString());return(d.toISOString());}
function ISOStringToICalStringZulu(ISOString){let ICALString=ISOString.replace(/\.\d\d\dZ/,"Z").replace(/\-|\:|\./g,"");return ICALString;}
...
function exportICSCalendar(){let eventsArr=makeVEventsArr(calendarDatabase["data"]);let cal=makeVCalendar(eventsArr);copyAndSaveString(cal,"ICSCalendar"+getTodaysDate(),".ics","text/calendar");}